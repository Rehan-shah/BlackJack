...
function Contorls() {
    ...
    const [compTotal , setComp] : [number , Function] = useContext(compTotalAmount);
...
useEffect(() => {setComp(getTotal(compArray)) ; setTotal(getTotal(userArray)); })
...
function check() {
    ...
    if (total <= 21 && (total === 21 || total > compTotal)) {
      setResult("win");
      setRoundWon((e) => (e +1));
...
function reset(){
    ...
    return (
        <div id="contorls" className=" grid grid-row-6 pl-8">
            <div> <div className="text-4xl  text-[#ded5d6] font-extrabold">{compTotal}</div></div>
            <div className='row-span-4'></div>
            <div className='mt-8'>
                <div className="text-4xl  text-[#ded5d6] font-extrabold">{total}</div>
                <button onClick={() => { setUserArray((e : card[]) => ([addCard() , ...e]))}}
...
const  Stats_SideBar = () => {
    ...
    <div className='my-12 '>
        <h2 className='text-3xl font-poppins font-bold py-2 text-[#ded5d6] text-right'>Win: <span className='text-[#c7b1c9]'>{roundWon}</span> </h2>
        <h2 className='text-3xl font-poppins font-bold py-2 text-[#ded5d6] text-right whitespace-nowrap'>lost: <span className='text-[#c7b1c9]'>{roundLost}</span> </h2>
        <h2 className='text-2xl font-poppins font-bold py-2 text-[#ded5d6] text-right'>Total: <span className='text-[#c7b1c9]'>${Total}</span> </h2>
    </div>
</div>)
};

 const The_game = () => {
    const [playerArray  ,setTestArray] : [unCard[] , Function] = useContext(CardArray) ;
    const [ compArray , setCompArray ] : [unCard[] , Function] =  useContext(CompCard);
    ...
    <div id="Main" className="col-span-4 grid row-span-3 pb-12  "  >
                    <div id="playerCard" className='flex mx-auto mb-auto'>
                        {compArray.map((item : card, i : number) => {
                            return (
                                <Card card={item} z_index={i} key={i} />
                            )
                        })}
                        <FlippedCard />
                    </div>
